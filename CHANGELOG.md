...
1. 新增联动了 `astrbot_plugin_Qzone` 插件，让 Bot 不仅能在群聊、私聊里分享内容，还能分享内容到QQ空间动态，支持LLM调用发空间说说！
2. 新增：每天60s读懂世界、AI资讯快报分享内容到QQ空间动态！
3. 新增了对 `astrbot_plugin_gitee_aiimg` 新版本的支持，并新增了**形象参考图逻辑**，允许 Bot 使用预设的参考图进行“图生图”来保持生活照的一致性。
4. 新增区分亲昵称呼和网名称呼，同时昵称与日程逻辑强化，强制 LLM 检查日程中是否包含用户昵称，防止 Bot 在和用户聊天时把用户当成第三人称（例如日程是“和张三吃饭”，Bot 对张三说“我今天要去和张三吃饭”的语病）。
5. 插件加载健壮性：优化了查找其他插件实例的逻辑，兼容 AstrBot 不同的加载状态。
6. 新增独立定时分享每天60s读懂世界、AI资讯快报，支持LLM触发。
7. 优化分享AI资讯快报逻辑，有时候源头没有数据会推送空白图片。
8. 新增夸克、36氪、51CTO、A站、爱范儿、网易、新浪、澎湃热搜。
9. 新增知识和推荐的内容库自定义。
10. 重写深度回溯获取聊天历史记录逻辑。
11. 新增指定LLM生成内容失败后自动降级使用默认的第一个模型逻辑。
12. 新增历史内容参考（引入过往动态记忆）。
13. 新增多时间段随机分享（避免固定模式）。
14. 新增“随机触发时间段” 和 “Cron触发后随机延迟” 两种产生随机时间点的方式统一存入数据库，并在启动时检查和恢复，从而彻底解决重启导致重复推送或跳过执行的问题。
15. 修复了奇奇怪怪的BUG。
